...
const UserList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      const updatedUsers = await Promise.all(usersData.map(async (user: User) => {
  ...
  return (
    <div className="w-full px-4">
      <div className="overflow-x-auto">
        <table className="min-w-full max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
            ...
            {users.map(user => (
              <tr key={user.id} className="border-t">
                <td className="px-6 py-4 text-center">
                  <Link to={`/users/${user.id}`} className="text-blue-500 hover:underline">{user.username}</Link>
                </td>
